...
Office.onReady(function(info){
  ...
  tag("open-editor").onclick = Jade.open_editor;
  ...
  tag("show-survey-row").onclick = function(){Jade.toggle_element('survey');tag('fb-type').focus();
...
//  static officeReady(info){// invoked when the office addin infrastructure has loaded
 ...
 //console.log(1)
   ...
   //console.log("initializing examples", tag(panel_name))
    ...
    div.innerHTML = '<button title="Save code to workbook" onclick="Jade.update_editor_script(\'' + panel_name + '\')">Save</button> <button id="' + panel_name + '_run" title="Save code to workbook and execute">Run</button> <select id="' + panel_name + '_function-names"></select>';
